...
function formatDateStr(dateStr) {
	...
	if ( dateStr.match('\-') ) {
		var date_split = dateStr.split( '-' );
		var curr_day = date_split[2];
		if ( curr_day.substring(0, 1) == 0 ) {
			curr_day = curr_day.substring(1);
		}
		var curr_month = date_split[1];
		if ( curr_month.substring(0, 1) == 0 ) {
			curr_month = curr_month.substring(1);
		}		
		var curr_year = date_split[0];		
	}
	
	var m_names = new Array("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec");	
	if ( curr_day && curr_month && curr_year ) {				
		var sup = "";
		...
		return m_names[--curr_month] + ' ' + curr_day + sup + ' ' + curr_year;
	}
}

function rssdisplaybox(RSS_id, divParentId, rssDate){
	var divId = 'rssinnerdiv';	
	this.rssDate = ( rssDate ? rssDate : '' );
	this.loadingHTML='<img src="rssbox/ajax-loader.gif" /> Loading RSS Incident Feed...';
	...
	if ( divParentId && document.getElementById(divParentId) ) {
		document.getElementById(divParentId).innerHTML = '<div id="'+divId+'-maincontainer"></div>';
	} else {
	...
	if ( rssDate ) {		
		document.getElementById('rss_display_date').innerHTML = formatDateStr(rssDate);
	}
}
...
var parameters="id="+encodeURIComponent(this.RSS_id)+"&cachetime="+this.cachetime+"&limit="+this.utotalitems+"&template="+this.template+"&bustcache="+new Date().getTime()+"&rssDate="+this.rssDate;
...
if ( ! rsscontent ) {
	rsscontent = "<div class=\"rsscontainer\" style=\"margin-left:15px;font-size:larger;font-style:italic;\">No incident data to display</div>";
}
