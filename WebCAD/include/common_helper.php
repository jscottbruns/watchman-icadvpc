...
function ini_decrypt() {
	...
	$ini_file = realpath( '/var/www/icad-poll/include/icad-config.pl' );
    ...
    $ini_array = parse_ini_string( `$ini_file`, 1 );
    ...
    foreach ( $config_array as $section => $value_array )
    {
        if ( isset($define_sections[$section]) )
        {
            while ( list($key, $val) = each($value_array) )
            {
                ...
                {
                	//fwrite($fh, strtoupper($define_sections[$section][$key]) . "=>$val\n");
                	if ( trim($val) )
	                    define( strtoupper($define_sections[$section][$key]), $val);
                }
                else
                {
                	//fwrite($fh, strtoupper($key) . "=>$val\n");
                	if ( trim($val) )
...
{
	$errfile = '/var/www/icad-poll/log/icad-poll.log';

	if ( $errfile )
	{
		if ( ! file_exists( dirname($errfile) ) )
		{
			mkdir( dirname($errfile), 0777, true);
		}
		if ( file_exists( dirname($errfile) ) && ! file_exists( $errfile) )
		{
			if (  ! touch($errfile) )
				unset($errfile);
		}
	...
	if ( ! $errfile )
		$errfile = realpath('/var/log/icad-poll.log');
